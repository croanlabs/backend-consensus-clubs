 ...
 *
 */
exp.findOrCreate = (username, externalInfo) => sequelize.transaction().then(tx => User.findOrCreate({
  where: {
    username,
  },
  defaults: {
 ...
 *
 */
exp.createUserBlockchain = username => eos.contract(config.eosUsername).then((contract) => {
 ...
 *
 */
exp.newReferral = referredBy => eos.contract(config.eosUsername).then((contract) => {
  const options = { authorization: [`${config.eosUsername}@active`] };
...
exp.getUserOpinions = async (userId, options = {}) => {
  ...
  const promises = userOpinions.rows.map(opinion => eosService
